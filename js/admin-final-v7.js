...
function showStatusModal(orderId, currentStatus) {
    const statuses = [
        { value: 'pending', label: 'În așteptare', icon: '⏳', description: 'Comandă nouă, așteaptă confirmare' },
        { value: 'confirmed', label: 'Confirmată', icon: '✅', description: 'Comandă confirmată de admin' },
        { value: 'processing', label: 'În procesare', icon: '⚙️', description: 'Comandă în curs de pregătire' },
        { value: 'shipped', label: 'Expediată', icon: '🚚', description: 'Comandă expediată către client' },
        { value: 'delivered', label: 'Livrată', icon: '🎉', description: 'Comandă livrată cu succes' },
        { value: 'cancelled', label: 'Anulată', icon: '❌', description: 'Comandă anulată' }
    ];
...
async function saveStatus(orderId) {
    ...
    console.log('🔄 Actualizare status comandă:', orderId, '→', status);
...
async function saveNewOrder(clientId) {
    ...
    // Găsește clientul pentru adresa de livrare
    const client = allClients.find(c => c.id === clientId);
    const shippingAddress = client ? 
        `${client.address || 'Adresă lipsă'}, ${client.city || ''}, ${client.county || ''}, ${client.country || 'România'}` :
        'Adresă de livrare lipsă';
    
    // Creează comanda în formatul așteptat de backend
    const orderData = {
        items: [
            {
                product_id: productType === 'Cupola' ? cupolaType : productType,
                product_name: productType === 'Cupola' ? `${cupolaType} - ${description}` : `${productType} - ${description}`,
                product_description: `${country} - ${description}${notes ? ' - ' + notes : ''}`,
                quantity: quantity,
        ...
        payment_method: 'card',
        notes: notes || `Comandă creată de admin. ${shippingDate ? 'Data livrare: ' + shippingDate : ''}`
    };
